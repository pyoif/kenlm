...
template <class M> void Continuation(const M &model) {
  ...
  CHECK_EQ(0, state.length);

  state = preserve;
  AppendTest("more", 5, -0.00181395, true);
  CHECK_EQ(4, state.length);
  AppendTest("loin", 5, -0.0432557, true);
  CHECK_EQ(1, state.length);
}
...
template <class M> void Blanks(const M &model) {
  ...
  CHECK_EQ(2, state.length);
  AppendTest("higher", 3, -1.509559 - 0.30103, false);
  CHECK_EQ(3, state.length);
...
template <class M> void ExtendLeftTest(const M &model) {
  ...
  CHECK_EQ(0, next_use);
  CHECK_EQ(little.extend_left, extend_none.extend_left);
  SLOPPY_CHECK_CLOSE(little.prob - little.rest, extend_none.prob, 0.001);
  CHECK_EQ(1, extend_none.ngram_length);
...
template <class M> void Stateless(const M &model) {
  ...
  CHECK_EQ(1, state.length);
  CHECK_EQ(static_cast<WordIndex>(0), state.words[0]);
}
...
template <class ModelT> void LoadingTest() {
  ...
  config.probing_multiplier = 2.0;
  {
    ...
    CHECK_EQ((WordIndex)37, m.GetVocabulary().Bound());
    Everything(m);
  }
  {
    ExpectEnumerateVocab enumerate;
...
TEST_CASE("quant_trie") {
  LoadingTest<QuantTrieModel>();
}
TEST_CASE("bhiksha_trie") {
  LoadingTest<ArrayTrieModel>();
}
TEST_CASE("quant_bhiksha_trie") {
...
template <class ModelT> void BinaryTest(Config::WriteMethod write_method) {
  ...
  ModelType type;
  REQUIRE(RecognizeBinary("test.binary", type));
  CHECK_EQ(ModelT::kModelType, type);
...
TEST_CASE("write_and_read_probing") {
  BinaryTest<ProbingModel>();
}
TEST_CASE("write_and_read_rest_probing") {
  BinaryTest<RestProbingModel>();
}
TEST_CASE("write_and_read_trie") {
  BinaryTest<TrieModel>();
}
TEST_CASE("write_and_read_quant_trie") {
  BinaryTest<QuantTrieModel>();
}
TEST_CASE("write_and_read_array_trie") {
  BinaryTest<ArrayTrieModel>();
}
TEST_CASE("write_and_read_quant_array_trie") {
